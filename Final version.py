...
def not_blank(question):
    while True:
        ...
        if response == "":
            ...
            print("Please enter a valid and alphabet only name. (eg. Amy, Ben, Candy)")
...
def sell_snacks():
    ...
    if not read_menu:
        ...
        if want_snack == "yes":
            snack_sales = 0
            show_menu()
            print(f"You can buy {MAX_SNACK} snacks")
...
def sell_tickets():
    ...
    try:
        ...
        num_tickets: int = num_check("How many tickets do you want to buy? (digits only) ")
        if num_tickets + ticket_sales > MAX_TICKETS:
            print(
                f"Sorry, you can only buy tickets up to {MAX_TICKETS - ticket_sales}."
            )
            print()
            time.sleep(1)
            return sell_tickets()
...
while ticket_sales < MAX_TICKETS:
    ...
    # Call function to choose payment method
        ...
        snack_frame['Total'] = snack_frame['Surcharge'] + snack_frame['Snack Price']
